...
import { getBonusesCa } from '@/lib/sanity'
...
export const metadata: Metadata = {
  title: `Best Casino Bonuses Canada ${new Date().getFullYear()}`,
  description: 'Find the best casino bonuses for Canadian players. Welcome bonuses, free spins and exclusive offers.',
  alternates: { canonical: CANONICAL },
}

export default async function CaBonusesPage() {
  const bonuses = await getBonusesCa(40).catch(() => [])
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', borderBottom: '1px solid var(--border)', padding: '40px 15px 32px' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          <Breadcrumbs crumbs={[{ label: 'Home', href: '/' }, { label: 'Canada', href: '/ca/' }, { label: 'Casino Bonuses' }]} />
          ...
          <p style={{ fontSize: '15px', color: 'var(--text-muted)' }}>The best current bonuses from top online casinos available to Canadian players.</p>
        </div>
      </div>
      ...
      <div className="section">
        {(bonuses as any[]).length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px', color: 'var(--text-faint)' }}>
            <p>No bonuses yet — add them in Sanity Studio under 🇨🇦 Canada.</p>
          </div>
        ) : (
              ...
              <div key={bonus._id} style={{
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                      {bonus.minimumIndbetaling != null && (
                        <div style={{ fontSize: '12px', color: 'var(--text-faint)' }}>
                          Min. deposit: <strong style={{ color: 'var(--text)' }}>CAD {bonus.minimumIndbetaling}</strong>
                        </div>
                      )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end', flexShrink: 0 }}>
                    ...
                    <Link href={`/ca/online-casino/bonus/${bonus.slug.current}`}
