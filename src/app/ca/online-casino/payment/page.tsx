...
export const metadata: Metadata = {
  title: `Online Casino Payment Methods Canada ${new Date().getFullYear()} — Compare Deposits & Withdrawals`,
  description: 'Compare all online casino payment methods available to Canadian players — fees, withdrawal times, and bonus eligibility.',
      ...
      <div style={{ background: 'var(--bg-hero)', borderBottom: '1px solid var(--border)', padding: '40px 15px 32px' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto' }}>
          ...
          <p style={{ fontSize: '15px', color: 'var(--text-muted)', maxWidth: '600px', margin: 0, lineHeight: 1.6 }}>
            Compare all payment methods available at Canadian online casinos — fees, withdrawal times, and bonus eligibility at a glance.
          </p>
        </div>
      </div>
      ...
      <div className="section">
        {(methods as any[]).length === 0 ? (
          <div style={{ textAlign: 'center', padding: '64px', color: 'var(--text-faint)' }}>
            <p>No payment methods yet — add them in Sanity Studio under 🇨🇦 Canada.</p>
          </div>
        ) : (
          <div style={{
            display: 'grid',
            ...
            {(methods as any[]).map((method: any) => (
              <div key={method._id} style={{
                ...
                <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  ...
                  <Link
                    href={`/ca/online-casino/payment/${method.slug.current}/`}
                    style={{
                      display: 'block',
                      textAlign: 'center',
                      padding: '10px 16px',
