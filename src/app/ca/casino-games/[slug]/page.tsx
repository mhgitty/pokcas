...
export async function generateStaticParams() {
  const games = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "casinoGame" && market == "ca" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const game = await getCasinoGameBySlugCa(slug).catch(() => null)
  if (!game) return {}
  const title = game.metaTitle || `${game.name} — Best Canadian Online Casinos`
  const description = game.metaDescription || `Find the best Canadian online casinos to play ${game.name}. Compare bonuses and sign up today.`
  const canonical = `${BASE}/ca/casino-games/${slug}/`
...
export default async function CaCasinoGamePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Casino Games', item: `${BASE}/ca/casino-games/` },
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            ...
            { label: 'Casino Games', href: '/ca/casino-games/' },
            { label: game.name },
          ]} />
        ...
        <div className="section">
          <div style={{ maxWidth: '860px', margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, marginBottom: '16px', color: 'var(--text)' }}>
              Canadian Casinos Where You Can Play {game.name}
            </h2>
