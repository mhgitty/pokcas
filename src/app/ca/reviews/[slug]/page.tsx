...
export async function generateStaticParams() {
  const bookmakers = await clientNoCdn.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "bookmaker" && market == "ca" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const bm = await getBookmakerBySlugCa(slug).catch(() => null)
  if (!bm) return {}
  const title = replaceDateVars(bm.metaTitle || `${bm.name} Review Canada — bonus & offers`)
  const description = replaceDateVars(bm.metaDescription || `Read our Canadian review of ${bm.name}. See bonus, wagering requirements and our rating.`)
  const canonical = `${BASE}/ca/reviews/${slug}/`
...
export default async function CaReviewSlugPage({ params }: Props) {
  ...
  const [bm, latestPosts, settings] = await Promise.all([
    getBookmakerBySlugCa(slug).catch(() => null),
    getPosts(6),
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 3, name: 'Casino Reviews', item: `${BASE}/ca/reviews/` },
                ...
                {bm.minIndbetaling != null && (
                  <div style={{ background: 'var(--bg-raised)', border: '1px solid var(--border)', borderRadius: '10px', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>CAD {bm.minIndbetaling}</div>
                    </div>
                  </div>
                )}
                {bm.gennemspilskrav && (
