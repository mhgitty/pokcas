...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!game) return {}
  const title = game.metaTitle || `${game.name} — Play at the Best Online Casinos`
  const description = game.metaDescription || `Find the best online casinos to play ${game.name}. Compare bonuses and sign up today.`
...
export default async function CasinoGamePage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Casino Games', item: `${BASE}/casino-games/` },
        ...
        <div className="section">
          <div style={{ maxWidth: '860px', margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, marginBottom: '16px', color: 'var(--text)' }}>
              Casinos Where You Can Play {game.name}
            </h2>
