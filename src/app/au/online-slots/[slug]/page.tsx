...
import { getSlotmachineBySlugAu, getPageByPathAu, getSiteSettings, getHreflangScript, getSlotmachinesForArchive, getArchiveFeaturedSlotIds, client } from '@/lib/sanity'
...
const ARCHIVE_MARKET = 'au' as const
const FLAG = '🇦🇺'
...
export async function generateStaticParams() {
  const slots = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "slotmachine" && market == "au" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export default async function SlotAUPage({ params }: Props) {
    ...
    return (
      <>
        <CmsPageView
          ...
          homeHref="/au/"
          lang="en-AU"
          canonical={`${BASE}/au/online-slots/${slug}/`}
          afterHero={archive}
        />
      </>
    )
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Online slots', item: `${BASE}/au/online-slots/` },
    ...
    <>
      ...
      <div style={{ background: 'var(--bg-hero)', paddingTop: '32px', paddingBottom: '32px', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '0 15px' }}>
          <Breadcrumbs crumbs={[
            { label: 'Home', href: '/au/' },
            { label: 'Online slots', href: '/au/online-slots/' },
            { label: label(slug) },
          ]} />
      ...
      <SlotDemo embed={slot.demoIframe} title={slot.demoIframeTitle} slotName={slot.name} provider={slot.provider} rtp={slot.rtp} promoCasino={slot.promoCasino} />
        ...
        {hasCasinos && (
          <div id="real-money" className="section" style={{ paddingBottom: 0, scrollMarginTop: '80px' }}>
            ...
            <CasinoComparisonTable casinos={slot.casinos} currency="A$" />
          </div>
        )}
