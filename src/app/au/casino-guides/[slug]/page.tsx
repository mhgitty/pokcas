...
import { getCasinoGuideBySlugAu, getSiteSettings, client } from '@/lib/sanity'
...
export async function generateStaticParams() {
  const guides = await client.fetch<Array<{ slug: { current: string } }>>(
    `*[_type == "casinoGuide" && market == "au" && defined(slug.current)] { slug }`
  ).catch(() => [])
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = await getCasinoGuideBySlugAu(slug).catch(() => null)
  if (!guide) return {}
  const canonical = `${BASE}/au/casino-guides/${slug}/`
...
export default async function AuCasinoGuidePage({ params }: Props) {
  ...
  const [guide, settings] = await Promise.all([
    getCasinoGuideBySlugAu(slug).catch(() => null),
  ...
  const breadcrumbs = [
    { label: 'Home', href: '/au/' },
    { label: 'Casino guides', href: '/au/casino-guides/' },
    { label: guide.title },
  ]
  ...
  const jsonLd = {
    ...
    '@graph': [
      ...
      { '@type': 'WebPage', '@id': `${canonical}#webpage`, url: canonical, name: replaceDateVars(guide.title), inLanguage: 'en-AU', publisher: { '@type': 'Organization', name: 'Pokcas', url: BASE } },
    ],
  }
